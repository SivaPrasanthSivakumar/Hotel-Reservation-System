...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (strtotime($check_in) < strtotime(date("Y-m-d"))) {
        echo json_encode(["available" => false, "reason" => "Check-in date cannot be in the past."]);
        exit;
    }

    // Validate dates
    if (strtotime($check_out) <= strtotime($check_in)) {
        echo json_encode(["available" => false, "reason" => "Check-out date must be later than check-in date."]);
        exit;
    }

    // Ensure numbers are positive
    if ($num_rooms <= 0 || $num_adults <= 0) {
        echo json_encode(["available" => false, "reason" => "Number of rooms and adults must be at least 1."]);
        exit;
    }
    ...
    if (count($rooms) < $num_rooms) {
        echo json_encode(["available" => false, "reason" => "Not enough available rooms."]);
        exit;
    }
    ...
    foreach ($rooms as $room) {
        ...
        if ($stmt->rowCount() > 0) {
            echo json_encode(["available" => false, "reason" => "Room $room is already reserved for the selected dates."]);
            exit;
        }
    }
    ...
    if (isset($_POST['check_availability'])) {
        $_SESSION['availability'] = ["available" => true];
        header("Location: booking.html?availability=checked");
        exit;
    }
    ...
    // Apply discount (50% off per child under 14)
    $child_discount = ($num_children * $price_per_night * 0.5);

    // Calculate total cost
    $total_cost = ($num_rooms * $price_per_night * $num_nights) - $child_discount;
